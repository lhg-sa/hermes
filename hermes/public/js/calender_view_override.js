...
frappe.views.Calendar = class Calendar {
    ...
    prepare_events(events) {
        ...
        return (events || []).map((d) => {
            ...
            // Show event on a single day if start or end date is invalid
            ...
            // Prepare the title with styled HTML
            d.title = `<div class="event-title">${frappe.utils.html2text(d.title)}</div>`;
            for (const other_detail of me.other_fields) {
                if (d[other_detail]) {
                    d.title += `<div class="event-detail">${frappe.utils.html2text(d[other_detail])}</div>`;
                }
            }
